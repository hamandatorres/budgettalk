...
const axiosInstance = axios.create({
    baseURL: process.env.REACT_APP_API_URL || "https://evening-cliffs-17109-56706eeb61a8.herokuapp.com",
...
export const createCouncilPerson = createAsyncThunk(
    ...
    async (formData) => (await axiosInstance.post('/api/councilperson', formData)).data
);

export const updateCouncilPerson = createAsyncThunk(
    'councilPerson/update',
    async ({ id, data }) => (await axiosInstance.put(`/api/councilperson/${id}`, data)).data
);
    ...
    reducers: {
        ...
        eliminateLoser: {
            ...
            reducer(state, { payload }) {
                ...
                const partySize = state.list.filter(p => p.party === payload.partyName).length;
                if (partySize > 2) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteAndReplaceCouncilPerson.fulfilled, (state, action) => {
                const idx = state.list.findIndex(p => p.id === action.payload.deleted.id);
                if (idx >= 0) {
                    if (action.payload.replacement) state.list[idx] = action.payload.replacement;
...
export { recordWin, eliminateLoser, fetchCouncilPersons, createCouncilPerson, updateCouncilPerson, deleteAndReplaceCouncilPerson };
export default councilPersonReducer;
